...
figspath_powtfs = os.path.join(figspath, "powtfs")
...
preamble = "\\let\\mypdfximage\\pdfximage\n\\def\\pdfximage{\\immediate\\mypdfximage}\n" \
    ...
    + "\hline \multicolumn{1}{|c|}{\\textbf{z(Ch 1 Power)}} & \multicolumn{1}{|c|}{\\textbf{z(Ch 2 Power)}}  & \multicolumn{1}{c|}{\\textbf{z(Forward GC)}} & \multicolumn{1}{c|}{\\textbf{z(Backward GC)}} & \multicolumn{1}{c|}{\\textbf{thresholded}} & \multicolumn{1}{c|}{\\textbf{thresholded}} \\\\ & & & & Forward GC & Backward GC \\\\ \hline\n" \
    + "\endfirsthead\n\n" \
    + "\multicolumn{6}{c}%\n" \
...
imgfiles_forward = glob(os.path.join(figspath_forward, 'z*.png'))
imgfiles_forward.sort()

imgfiles_backward = glob(os.path.join(figspath_backward, 'z*.png'))
imgfiles_backward.sort()

imgfiles_powtfs = glob(os.path.join(figspath_powtfs, 'z*.png'))
imgfiles_powtfs.sort()

thresh_forward = glob(os.path.join(figspath_forward, 'thresh*.png'))
thresh_forward.sort()

thresh_backward = glob(os.path.join(figspath_backward, 'thresh*.png'))
thresh_backward.sort()
...
for i,f in enumerate(imgfiles_forward):
    ...
    pow1f = imgfiles_powtfs[pairs[i][0]]
    pow2f = imgfiles_powtfs[pairs[i][1]]
    thisline = \
        "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + pow1f + "}\n\end{tabular}\n&\n" \
        "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + pow2f + "}\n\end{tabular}\n&\n" \
        + "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + imgfiles_forward[i] + "}\n\end{tabular}\n&\n" \
        + "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + imgfiles_backward[i] + "}\n\end{tabular}\n&\n" \
        + "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + thresh_forward[i] + "}\n\end{tabular}\n&\n" \
        + "\\begin{tabular}{c}\n\\includegraphics[width=" \
        + str(imgsize) + "\\textwidth]{" + thresh_backward[i] + "}\n\end{tabular}\n\\\\\n"
    texf.write(thisline)
